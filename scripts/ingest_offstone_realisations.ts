// @ts-nocheck
/*
  Ingestion script: crawls https://offstone.fr/nos-realisations/ and child pages.
  ...
  Run locally (Node >= 18):
    ...
    node --loader ts-node/esm scripts/ingest_offstone_realisations.ts  (or use tsx)
...
async function extractOne(url: string, cheerio: CheerioStatic, sharp: SharpModule | null) {
  ...
  const title = normalizeText($("h1").first().text()) || normalizeText($("title").text()) || "Référence Offstone";

...
export async function generateMetadata({ params }: { params: { uid: string } }): Promise<Metadata> {
  const canonical = toCanonical(`/ressources/jonathan-anguelov/interviews/${params.uid}`);
  return { title: "Interview | Jonathan Anguelov", alternates: { canonical } };
}
...
export default async function InterviewDetailPage({ params }: { params: { uid: string } }) {
  const client = createClient();
  const doc = await client.getByUID("press_item" as any, params.uid).catch(() => null);
  const data = (doc?.data || {}) as PressItemData;
  if (!doc || data.section !== "interview") return <div>Not found</div>;
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-14">
        ...
        {data.external_url && <a href={data.external_url} target="_blank" rel="noopener" className="block text-blue-600 underline mt-4">Lire l’interview original</a>}
      </main>

...
export async function generateMetadata(): Promise<Metadata> {
  const title = "Interviews | Jonathan Anguelov | Offstone";
  const description = "Interviews de Jonathan Anguelov.";
  const canonical = toCanonical("/ressources/jonathan-anguelov/interviews");
  return { title, description, alternates: { canonical } };
}

export default async function InterviewsIndexPage() {
  const client = createClient();
  // On filtre les press_item avec section = "interview"
  const items = (await client.getAllByType("press_item" as any, {
    filters: [prismic.filter.at("my.press_item.section", "interview")],
  ...
  const breadcrumbData = breadcrumbLD([
    ...
    { name: "Interviews", url: toCanonical("/ressources/jonathan-anguelov/interviews") },
  ]);
  const collectionLD = collectionPageLD({ url: toCanonical("/ressources/jonathan-anguelov/interviews"), name: "Interviews", description: "Interviews de Jonathan Anguelov" });

  return (
    <>
      <Navbar forceWhiteStyle />
      <main className="mx-auto max-w-5xl px-4 py-14">
        <h1 className="text-4xl md:text-5xl font-bold mb-8 tracking-tight">Interviews</h1>
        <ul className="space-y-4">
          {items.map((it) => (
            <li key={it.id} className="p-4 rounded-lg border bg-white shadow-sm">
              <a href={`/ressources/jonathan-anguelov/interviews/${it.uid}`} className="text-lg font-semibold text-blue-700 hover:underline">

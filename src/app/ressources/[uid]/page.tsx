...
export default async function ArticlePage({ params }: { params: Promise<Params> }) {
  ...
  const canonicalPath = `/ressources/${uid}`;
  ...
  if (catUid) breadcrumbItems.push({ name: catUid, url: toCanonical(`/ressources/${catUid}`) });
  breadcrumbItems.push({ name: data.title || "Article", url: toCanonical(canonicalPath) });
  ...
  const authorRole = Array.isArray(data.authors) && data.authors.length > 0 
    ? data.authors[0].role 
    : null;
              ...
              <ol className="flex items-center space-x-2 text-gray-500">
                ...
                {catUid && (
                  <li className="flex items-center">
                    ...
                    <Link href={`/ressources/${catUid}`} className="hover:text-blue-600 transition-colors">{catUid}</Link>
                  </li>
                )}
                <li className="flex items-center">
                  <svg className="w-4 h-4 mx-2" fill="currentColor" viewBox="0 0 20 20">

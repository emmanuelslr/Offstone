...
function HeroPourquoiOffstoneInner(props: any) {
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-8 md:gap-10 lg:gap-0">
            ...
            <div className="w-full lg:w-1/2 lg:pr-12 lg:mb-0 mt-8 md:mt-20 lg:mt-28">
                ...
                onSubmit={(e) => {
                  ...
                  try {
                    ...
                    (function(d){ if (typeof window !== 'undefined') { try { const w:any = window as any; if (w.offstoneOpenWaitlist) { w.offstoneOpenWaitlist(d); } else { (w.__offstone_waitlist_queue ||= []).push(d); w.dispatchEvent(new CustomEvent('waitlist:open', { detail: d })); } } catch(e){} } })({
                      ...
                      utm_content: params.get('utm_content') || 'hero-pourquoi-offstone',
              ...
              >
                ...
                <button
                  type="submit"
                  className="inline-flex items-center justify-center w-full sm:w-auto px-4 xs:px-5 py-2.5 xs:py-3 bg-black text-white border border-black rounded-lg font-medium transition hover:bg-[#F7B096] hover:text-black hover:border-[#F7B096] text-xs xs:text-sm whitespace-nowrap group"
                >
                  Candidatez
                  <svg
...
export default function HeroPourquoiOffstone(props: any) {
  return (
    <Suspense>
      <HeroPourquoiOffstoneInner {...props} />
    </Suspense>
  );
}

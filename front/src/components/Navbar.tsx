...
export default function Navbar() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach(entry => {
          // Si une section blanche devient visible
          if (entry.isIntersecting && !entry.target.classList.contains('dark-section') && !entry.target.classList.contains('Hero')) {
            setIsOnWhiteSection(true);
          }
          // Si une section sombre devient visible
          else if (entry.isIntersecting && (entry.target.classList.contains('dark-section') || entry.target.classList.contains('Hero'))) {
    ...
    // Observer toutes les sections à fond sombre
    const darkSections = document.querySelectorAll('.dark-section, .Hero');
    darkSections.forEach(section => {
      observer.observe(section);
    });

    // Observer la section TextReveal
    const textReveal = document.querySelector('.TextReveal');
    if (textReveal) {
  ...
  return (
    <header
      className={`fixed top-4 inset-x-4 lg:inset-x-8 mx-auto max-w-[96rem] z-50 rounded-2xl transition-all duration-300 ${
        isOnWhiteSection ? 'bg-black/5' : 'bg-white/5'
      } backdrop-blur-md shadow-[0_2px_6px_0_rgba(0,0,0,0.05),0_6px_16px_0_rgba(0,0,0,0.025)]`}
    >
      <nav className="mx-auto max-w-[92rem] px-10 flex items-center justify-between h-20">
          ...
          <svg width="220" height="35" viewBox="0 0 372 110" className="group">
            <path 
              ...
              className={`transition-all duration-300 ${
                isOnWhiteSection ? 'fill-black group-hover:fill-[#4E6AE9]' : 'fill-white group-hover:fill-[#4E6AE9]'
              }`}
            />
            <path 
              ...
              className={`transition-all duration-300 ${isOnWhiteSection ? 'fill-black' : 'fill-white'}`}
            />
          </svg>
        </Link>
        ...
        <ul className="hidden md:flex items-center space-x-12">
          ...
          <li>
            <Link href="#demo" className="inline-flex items-center justify-center px-6 py-3 text-[15px] font-normal tracking-[-0.01em] text-black bg-white border border-white rounded hover:bg-[#1E2124] hover:text-white transition-all">
              Request Demo
            </Link>
          </li>
        </ul>
        ...
        <button className="md:hidden p-2" aria-label="Menu">
          <svg
            className={`w-6 h-6 transition-all duration-300 ${isOnWhiteSection ? 'text-black' : 'text-white'}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
